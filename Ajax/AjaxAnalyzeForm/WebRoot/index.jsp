        ...
        <title>理财信息</title>
         ...
         <script type="text/javascript">
		  		$("input[type=button]").click(function(){
		  			//序列化:没有被禁用,必须有name属性,选中的check或radio,触发submit,file不会被序列化
		  			var form=$("form[name=form]").serializeArray();
		  			//alert(form);
		  			//alert(form.length);
		  			//form=$.param(form);//转成字符串
		  			//alert(form);
		  			//alert(form.length);
		  			//$(form).each(function(){
		  				//alert(this);
		  			...
		  			//alert(form);
		  			//alert(form.length);
		  			//for ( var int = 0; int < form.length; int++) {
						//alert(form[int]);
					//}
		  			...
		  			url="FindFinancingProductServlet";
		  			
		  			//$.get(url[,data][,success][,dataType]);
		  			//$.post(url[,data][,success][,dataType]);
		  			//$.getJSON(url[,data][,success][,dataType]);默认返回JSON
		  			$.getJSON(url,form,function(json){
